...
//int left_threshold;
//int left_read;
// Line follow vars for new sensor
QTRSensors qtr;
//int set = 0;
double kPline = 0.05;
double kDline = 0.005;
...
void setup() {  
  ...
  pinMode(A6,INPUT);
  qtr.setTypeRC();
  qtr.setSensorPins((const uint8_t[]){A4, A5, A6}, SensorCount);
...
//        moveForward(); //runs move forward function from above
        ...
        digitalWrite(motor0pin1, HIGH); 
        digitalWrite(motor1pin2, LOW); //left motor, backward
        digitalWrite(motor1pin1, HIGH); 
//        delay(6000);
